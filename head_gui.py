import os
from PyQt5.QtWidgets import QListWidget, QListWidgetItem, QWidget, QVBoxLayout, QGraphicsBlurEffect, QGraphicsOpacityEffect, QLabel
...
class FrostedGlassWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.blur_effect.setBlurRadius(20)  # 增大模糊半径

        self.opacity_effect = QGraphicsOpacityEffect()
        self.opacity_effect.setOpacity(0.65)  # 设置透明度为65%
...
class TabControl(QWidget):
    def __init__(self, stacked_widget):
        ...
        # 创建背景部件并应用模糊和透明效果
        self.background_widget = FrostedGlassWidget(self)
        self.background_label = QLabel(self.background_widget)
        self.background_label.setPixmap(QPixmap(get_full_path("background_image.png")))
        self.background_label.setScaledContents(True)
        self.background_label.setFixedSize(self.expanded_width, self.height())
    ...
    def create_tabs(self):
        ...
        # 创建主要设置标签
        for index, (name, icon) in enumerate(self.main_tab.items(), start=len(self.tabs) + 1):
            item = QListWidgetItem(name)
            item.setTextAlignment(Qt.AlignCenter)
            item.setIcon(QIcon(get_full_path(f"icons/{icon}")))
            self.list_widget.addItem(item)
    ...
    def perform_resize(self):
        ...
        if self.current_step <= self.animation_steps:
            ...
            self.background_label.setFixedWidth(self.end_width)
    ...
    def set_initial_state(self):
        ...
        self.background_label.setFixedWidth(self.expanded_width)

    def apply_frosted_glass_effect(self, widget):
        """应用毛玻璃效果"""
        blur_effect = QGraphicsBlurEffect()
        blur_effect.setBlurRadius(20)  # 增大模糊半径
        widget.setGraphicsEffect(blur_effect)

        opacity_effect = QGraphicsOpacityEffect()
        opacity_effect.setOpacity(0.65)  # 设置透明度为65%
        widget.setGraphicsEffect(opacity_effect)

# 示例使用
# stacked_widget = QStackedWidget()
# tab_control = TabControl(stacked_widget)
