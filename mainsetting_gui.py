...
def edit_selected_item(tree_widget, local_folder_path):
    ...
    # 显示提示信息
    QMessageBox.warning(tree_widget, "警告", "修改功能有可能导致未刷新的日志丢失，建议您在修改前手动更新日志文件。")
    ...
    try:
        ...
        for log_file in log_files:
            ...
            for entry in log_entries:
                if entry.get('timestamp') == timestamp:  # 找到对应的日志条目
                    player_info_split = player_info.split(' (')
                    if len(player_info_split) == 2:
                        entry['player_name'], entry['player_number'] = player_info_split[0], player_info_split[1][:-1]
                    entry['command'] = command
                    entry['parameters'] = parameters
                    save_admin_log_file(os.path.join(local_folder_path, log_file), log_entries)  # 保存修改后的日志文件
                    print(f"修改日志文件: {log_file} - 时间戳: {timestamp}, 玩家信息: {player_info}, 命令: {command}, 参数: {parameters}")
        QMessageBox.information(tree_widget, "成功", "日志文件修改成功。")
    except Exception as e:
        print(f"修改日志文件失败: {e}")
        QMessageBox.critical(tree_widget, "错误", "日志文件修改失败。")
